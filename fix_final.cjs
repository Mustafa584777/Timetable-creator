...
// wrap_classicConfigDays logic (which starts with 1 Day ... 4 Days without custom icons, then 5, 6, 7 with icons? No wait)
html = html.replace(
...
// Add 1..4 to wrap_configDays (pro mode)
html = html.replace(
...
// We need to remove the bad block in wrap_configStart:
html = html.replace(
